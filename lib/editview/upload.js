...
function processFile (args, file, type, cb) {
    ...
    fs.readFile(source, function (err, data) {
        ...
        getName(target, function (target, fname) {
            ...
            if (type.useUploadAdapter) {
                if (args.custom.adapters != null) {
                    var uploadAdapterPath = args.custom.adapters.uploadAdapter;
                    if (uploadAdapterPath){
                        var fullPath = path.join(args.dpath, uploadAdapterPath);
                        if (fs.existsSync(fullPath)) {
                            uploadAdapter = require(fullPath);
                        }
                    }
                }
            }
            if (uploadAdapter) {
                uploadAdapter.upload({args, file, type, target, data, fname, cb})
            } else {
